...
\end{remark}
\begin{proof}
We do it for $u \in \mc S$ and we need to show $\|v\|_{H^{s - 1/2}(\R^{n-1})} \le C \|u\|_{H^s(\R^n)}$.  

Note that $$\hat{v}(\eta) = \int_{\R^{n-1} u(0, y) e^{-y \cdot \eta}\, dy}.$$

Then, using the Fourier Inversion formula
$$u(0) = \int_\R \hat{u}(\xi)\, d\xi.$$
 
 Hence,
 $$\hat{v}(\eta) = \int u(\xi_1, y) e^{-i y \cdot \eta} d\xi_1 dy = \int \hat{u}(\xi_1, \eta) d\xi_1.$$
 
 Note that 
 $$\|v\|_{H^{s-1/2}}^2 = \int_{\R^{n-1}} \<\eta\>^{2s-1} |\hat{v}(\xi)|^2\, d\eta = \int_{\R^{n-1}} \<\eta\>^{2s-1} \left |\int \hat{u}(\xi_1, y)\, d\xi_1\right |^2d\eta.$$
 
 Then,
 \begin{align*}
 \int_{\R^{n-1}} \<\eta\>^{2s-1} \left |\int \hat{u}(\xi_1, y)\, d\xi_1\right |^2d\eta = \int \<\eta\>^{2s-1} \left |\hat{u}(\xi, \eta)(1 + |\xi_1| + |\eta|)^{s/2} (1 + |\xi_1| + |\eta|)^{-s/2}d\xi_1  \right |d\eta \\
 \le \int \<\eta^{2s-1}\> \int |\hat{u}(\xi_1, \eta)|^2 (1 + |\xi_1|^2 + |\eta|^2)^s d\xi_1 \int (1 + |\xi_1|^2 + |\xi|^2)^{-s} d\xi_1\, d\eta \\
\le \int \<\eta\>^{2s-1} \<\eta\>^{-2s+1}  \int |\hat{u}(\xi_1, \eta)|^2 (1 + |\xi_1|^2 + |\eta|^2)^s d\xi_1\int (1 + u^2)^{-s} du \\
 = \int |\hat{u}(\xi)|^2 \<\xi\>^{2s} d\xi = \|u\|_{H^s}^2.
 \end{align*}
 \begin{remark} Warning: the above computation is probably full of typos.  
 \end{remark}
\end{proof}
