...
\begin{example}[Rooks on chessboards] For $n$ fixed, a chessboard $C$ is a subset of $[n] \times [n]$.  We are given $C$, and we defined a sequence $\{r_k\}$ where $r_k$ is the number of ways we can place $k$ nonattacking rooks on $C$.
