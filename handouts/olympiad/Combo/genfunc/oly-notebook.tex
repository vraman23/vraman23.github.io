...
\begin{definition}[ Power Mean] Given $p \in \R$, $x_1, \dots, x_n \in \R^+$, define 
...
\end{definition}
\begin{definition}[Weighted Power Mean] Given $(w_i)_{i=1}^n$ with $\sum_i w_i = 1$, define 
...
\begin{theorem}[Incenter-Excenter Lemma]
\end{theorem}
\begin{theorem}[Euler's Theorem] Let $ABC$ be a triangle.  Let $R$ and $r$ denote its circumradius and inradius, respectively.  Let $O$ and $I$ denote the circumcenter and incenter.  then $OI^2 = R(R - 2r)$.  In particular, $R \ge 2r$.
\end{theorem}
\subsection{Complex Numbers}
\begin{theorem}[Complex Special Points]
...
\end{itemize}
\end{theorem}
\begin{theorem}[Complex Incenter] Given $ABC$ on the unit circle, it is possible to pick $u, v, w$ such that 
\begin{itemize}
...
\end{itemize}
\end{theorem}
\begin{theorem}[Complex Foot] If $a \ne b$ are on the unit circle and $z \in \C$, then the foot from $Z$ to $AB$ is given by 
$$\frac{a + b + z - ab\bar{z}}{2 }$$
\end{theorem}

\begin{theorem}[Complex Shoelace] If $a, b, c \in \C$, the signed area of $\triangle ABC$ is given by 
...
\end{vmatrix}.$$
\end{theorem}
\begin{theorem}[Concyclic Complex Numbers] Let $a, b, c, d$ be distinct complex numbers, not all collinear.  Then $A, B, C, D$ are concyclic if and only if 
